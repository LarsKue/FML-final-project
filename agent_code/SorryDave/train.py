...
def setup_training(self):
    ...
    if self.evaluate:
        ...
        self.agent = SorryDaveAgent(train=False, weights=self.weights[0])
    else:
        self.agent = SorryDaveAgent(train=True)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    if self.evaluate:
        ...
        if self.current_round_number == self.rounds_per_weight:
            ...
            self.agent = SorryDaveAgent(train=False, weights=self.weights[self.current_weight_index])
